...
if numcomputers>32 | strcmp(bram_table,'on'),
    ...
    reuse_block(blk,'ip_table','xps_library/Shared BRAM',...
                'arith_type','Unsigned',...
                'addr_width','11',...
                'init_vals',['1:',num2str(numcomputers)],...
                'Position',[355    55   455    85]);
    reuse_block(blk,'port_table','xps_library/Shared BRAM',...
                'arith_type','Unsigned',...
                'addr_width','11',...
                'init_vals',['1:',num2str(numcomputers)],...
                'Position',[355   140   455   170]);

    add_line(blk, 'data_in/1', 'ip_table/2');
    add_line(blk, 'we/1', 'ip_table/3');
    add_line(blk, 'data_in/1', 'port_table/2');
    add_line(blk, 'we/1', 'port_table/3');

    add_line(blk,'Counter/1','ip_table/1');
    add_line(blk,'Counter/1','port_table/1');
    add_line(blk,'ip_table/1','IP/1');
    add_line(blk,'ip_table/1','Compareto0/2');
    add_line(blk,'port_table/1','PortSlice/1');
else,
    ...
    for i=1:numcomputers,
        %add ip registers
        reuse_block(blk,['reg_ip',num2str(i-1)],'xps_library/software register',...
            'io_dir','From Processor',...
            'Position',[85 45+i*45 185 75+i*45]);
        reuse_block(blk,['IP_Constant',num2str(i-1)],'simulink/Sources/Constant',...
            'Value', num2str(4000+i),...
            'Position',[25 45+i*45 55 75+i*45]);
        add_line(blk,['IP_Constant',num2str(i-1),'/1'],['reg_ip',num2str(i-1),'/1']);
        add_line(blk,['reg_ip',num2str(i-1),'/1'],['IPMux/',num2str(i+1)]);
        
        %add port registers
        reuse_block(blk,['reg_port',num2str(i-1)],'xps_library/software register',...
            'io_dir','From Processor',...
            'Position',[85 45+i*45+60*numcomputers 185 75+i*45+60*numcomputers]);
        reuse_block(blk,['Port_Constant',num2str(i-1)],'simulink/Sources/Constant',...
            'Value', '4000',...
            'Position',[25 45+i*45+60*numcomputers 55 75+i*45+60*numcomputers]);
        add_line(blk,['Port_Constant',num2str(i-1),'/1'],['reg_port',num2str(i-1),'/1']);
        add_line(blk,['reg_port',num2str(i-1),'/1'],['PortMux/',num2str(i+1)]);
    end
end
