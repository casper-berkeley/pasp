...
const0 = xSignal;
xBlock('Constant',...
       struct('const',0,'arith_type','Unsigned','n_bits',32,'bin_pt',0,'ShowName','off','explicit_period','on'),...
       {},{const0});
...
if numcomputers>32 | strcmp(bram_table,'on'),
    data_in = xSignal;
    xBlock('Constant', struct('n_bits',32,'arith_type','Unsigned','explicit_period','on'),{},{data_in});

    write_enable = xSignal;
    xBlock('Constant', struct('arith_type','Boolean','const',0,'explicit_period','on'),{},{write_enable});
    ...
    for i=1:numcomputers,
        ...
        xBlock(struct('source','xps_library/software register','name',['reg_ip',num2str(i-1)]),...
               ...
               {ip_constant},{ip_reg});
        xBlock('Constant',struct('const',4000+i,'bin_pt',0,'explicit_period','on'),{},{ip_constant});

        xBlock(struct('source','xps_library/software register','name',['reg_port',num2str(i-1)]),...
               struct('io_dir','From Processor'),...
               {port_constant},{port_reg});
        xBlock('Constant',struct('const',4000+i,'bin_pt',0,'explicit_period','on'),{},{port_constant});
